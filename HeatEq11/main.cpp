...
void Menu(HeatEq Eq) {
	...
	do {
		...
		if (com == "info") {
			cout << "¬ариант 4" << endl;
			cout << "”равнение тепопроводности: du/dt = 9 * d^2u/dx^2 + 5*sin(t)" << endl;
			cout << "x принадлежит отрезку [0,1], t принадлежит отрезку [0, 1000]" << endl;
			cout << "Ќачальные услови€: u(x, 0) = 1 - x^2" << endl;
			cout << "√раничные услови€ слева (второго рода): du/dx(0, t) = 0" << endl;
			cout << "√раничные услови€ справа (третьего рода): -du/dx(1, t) = 7 * (u(1, t) - 2/7)" << endl;
			cout << "Ќе€вна€ разностна€ схема:" << endl;
			...
			cout << "ћатрица прогонки дл€ последнего посчитанного сло€" << endl;
			Eq.PrintMatrix();*/
		}
		...
		if (com == "temp") {
			...
			string strx, strt;
			while ((x < 0) || (x > 1)) {
				cout << "¬едите x (от 0 до 1): ";
				cin >> x;
			}

			while ((t < 0) || (t > 1000)) {
				cout << "¬едите t (от 0 до 1000): ";
				cin >> t;
			}
		...
		if (com == "layer") {
			int num;
			cout << "¬ведите номер сло€ от 0 до " << Eq.Getm() << ": ";
			cin >> num;
			Eq.ZeroLayer();
			if (num == 0) {
				Eq.PrintLastLayer();
				...
				for (int i = 1; i <= num; i++) {
					Eq.NextLayer();
					//cout << "ѕосчитал " << layer_counter << " слой" << endl;
				}
				Eq.PrintLastLayer();
			}
		}
		...
		if (com == "help") {
			cout << endl;
			cout << "initial - ввод новых параметров дл€ уравнени€" << endl;
			...
			cout << "matrix - напечатать матрицу прогонки (дл€ последнего посчитанного сло€)" << endl;*/
			cout << "exit - exit" << endl;
		}
	} while (com != "exit");
}
...
int main() {
	system("title «адача 10 Ќе€вна€ разностна€ схема дл€ нестационарного уравнени€ теплопроводности");
